...
## The pattern generalises past code

The map exists because reading files to answer structural questions is slow and expensive.
The same trade applies to any artifact a model would otherwise consume raw - screen
recordings, PDFs, long transcripts, design files. Build a small generated index (structure,
positions, machine-checkable properties, plus ONE cheap excerpt per unit) and let the model
spend its attention on the judgement that only judgement settles.

Worked example: a help-video reviewer that maps each recording as one entry per caption
window with its clicks, frame statistics and flags, plus a half-resolution keyframe. Reading
every frame at full resolution had cost ~6.5M tokens; the map cut the review payload by ~90%.
Two rules carried over from the code map and both proved essential: the index must be
GENERATED by the tool that produces the artifact (re-deriving caption positions from the
source clock broke the moment the assembler started trimming), and every detector must be
pinned to real defects in a self-test, or the cheap path rots without anyone noticing.
...
Two lessons from a 2026-09 sweep that removed 52 files. A barrel `index.ts` that
nothing imports is itself an orphan and hides everything behind it; the mapper no
longer counts deep barrels as entries, so if one shows up, check its members. And
after deleting orphans, **rebuild before reporting deps**: a package only the deleted
files imported appears as a new `unusedDeps` entry, and the snapshot has no per-file
import list to predict that from.
...
## Mode 6: Marketing-claims gate (before any production launch)

When a feature round is heading to production in a repo that has a public
marketing surface, run this gate before the ship. It exists because of a
2026-08 audit that found 24 live claim/reality mismatches on one product site,
four of them legal-risk, and every one of them entered silently while shipping
features. The map makes this cheap: `--find` and `--trace` locate the marketing
routes and the enforcement code without reading the whole repo.

Sweep the public pages (marketing routes, pricing, FAQ, comparison pages,
privacy/terms) for the four drift classes, checking each against code truth:

1. **Tier and price drift.** Every "from $X/mo" and every feature-availability
   claim on a landing page must match the enforced tier matrix (the constants
   or plan config the gates actually read), not memory. If a pricing table
   renders from a database, it must go through a code-truth overlay or a drift
   test; DB feature rows and code constants WILL diverge silently otherwise.
2. **Billing-flow drift.** Trial length, card requirements, auto-charge,
   cancellation, and refund copy must match the live billing flags and
   webhook behavior. "No credit card required" surviving a card-gate launch
   is a false-advertising (ROSCA) exposure, not a nit.
3. **Capability drift, in both directions.** Claims of features that do not
   exist, AND your own comparison pages denying features that DO exist. When a
   round ships or re-tiers a feature, grep the marketing surface for that
   feature's name and re-verify every hit.
4. **Legal-surface drift.** Privacy/terms statements about tracking must match
   the scripts actually loaded (grep for analytics/pixel mounts in the layout);
   testimonials and rating badges must have a real, citable source; legal
   compliance claims ("valid in all 50 states") must be hedged unless the code
   actually implements them.

Also check structured data (JSON-LD offers vs real plans, no fabricated
aggregateRating) and canonicals on any page whose metadata changed.

Report findings with the page, the exact quoted claim, the code evidence, and a
proposed fix, ranked: false/legal-risk first, misleading second, understated
third. Block the launch summary on the first two classes until each is fixed or
